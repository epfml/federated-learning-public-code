...
class Timer:
    ...
    def __init__(self, verbosity_level=1, log_fn=None, skip_first=False, on_cuda=True):
    ...
    def summary(self):
        ...
        if len(self.totals) > 0:
            with StringIO() as buffer:
                ...
                print(
                    f"  {'Event':30s} | {'Count':6s} | {'Average time':5s} | {'Frac':5s}",
                    file=buffer,
                )
                ...
                print("-------------------------------------------", file=buffer)
                print(
                    f"- {'total_averaged_time':30s}| {count:6d} | {total_avg_time:11.5f}s |",
                    file=buffer,
                )
                print("-------------------------------------------", file=buffer)
