...
def generate_data(
    random_state, batch_size, num_images_per_classes, device, output_path
):
...
def one_hot_from_names(rng_state=None, class_name_or_list=None, batch_size=1):
    ...
    if class_name_or_list is not None:
        ...
        for class_name in class_name_or_list:
            ...
            original_synsets = list(
                filter(lambda s: s.pos() == "n", original_synsets)
            )  # keep only names
            if not original_synsets:
                return None

            possible_synsets = list(
...
if __name__ == "__main__":
    seed = 7
    batch_size = 200
    num_images_per_classes = 1000
    device = torch.device("cuda:0")
    output_path = "./data/pseudo_imagenet"
    random_state = np.random.RandomState(seed)

    # generate data
    generate_data(random_state, batch_size, num_images_per_classes, device, output_path)
